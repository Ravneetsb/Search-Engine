...
/**
 * Demonstrates how to use the built-in {@link HttpClient} to fetch the headers and content from a
 * URI on the web, as well as how to setup the {@link Socket} connections to do the same manually
 * (well suited for web crawling).
 *
...
public class HttpsFetcher {
  /**
   * Fetches the headers and content for the specified URI. The content is placed as a list of all
  ...
  public static Map<String, List<String>> fetch(URI uri) throws IOException {
    try (Socket socket = openConnection(uri);
  ...
  /**
   * Uses a {@link Socket} to open a connection to the web server associated with the provided URI.
   ...
   * @see HttpHeaders#map()
   */
  public static Map<String, List<String>> processHttpHeaders(BufferedReader response)
      throws IOException {
