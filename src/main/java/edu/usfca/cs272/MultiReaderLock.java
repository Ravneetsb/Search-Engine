 ...
 * write lock is exclusive. The active writer is able to acquire read or write locks as long as it
 * is active.
...
public class MultiReaderLock {
  ...
  /**
   * Determines whether the thread running this code and the writer thread are in fact the same
   * thread.
   *
   * @return true if the thread running this code and the writer thread are not null and are the
   *     same thread
  ...
  /**
   * A simple lock used for conditional synchronization as an alternative to using a {@code
   * synchronized} block.
   *
   * <p>Similar but simpler than {@link Lock}.
   *
   * @author CS 272 Software Development (University of San Francisco)
   * @version Spring 2024
   */
  ...
  private class ReadLock implements SimpleLock {
    /**
     * Controls access to the read lock. The active thread is forced to wait while there are any
     * active writers and it is not the active writer thread. Once safe, the thread is allowed to
  ...
  private class WriteLock implements SimpleLock {
    /**
     * Controls access to the write lock. The active thread is forced to wait while there are any
     * active readers or writers, and it is not the active writer thread. Once safe, the thread is
     * allowed to acquire a write lock by incrementing the number of active writers and setting the
    ...
    /**
     * Will decrease the number of active writers and notify any waiting threads if necessary. Also
     * unsets the active writer if appropriate.
     *
     * @throws IllegalStateException if no writers to unlock
     * @throws ConcurrentModificationException if there are writers but unlock is called by a thread
    ...
    public void unlock() throws IllegalStateException, ConcurrentModificationException {
      synchronized (lock) {
        ...
        if (!isActiveWriter()) {
          throw new ConcurrentModificationException("Thread doesn't own the lock.");
        }
        writers--;
        if (writers == 0) {
          activeWriter = null;
