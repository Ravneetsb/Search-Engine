...
public class ArgumentParser {
  ...
  /**
   * Determines whether the argument is a flag. The argument is considered a flag if it is a dash
   ...
   * values. If a flag is repeated, its value will be overwritten.
   *
   * @param args the command line arguments to parse
   */
  public void parse(String[] args) {
    ...
    for (i = 1; i < args.length; i++) {
      if (isFlag(args[i - 1]) && isValue(args[i])) {
        this.map.put(args[i - 1], args[i]);
      } else if (isFlag(args[i - 1])) this.map.put(args[i - 1], null);
    }
    if (isFlag(args[i - 1])) {
      this.map.put(args[i - 1], null);
    }
  }
  ...
  /**
   * Returns the value to which the specified flag is mapped as a {@link String} or the backup value
   * if there is no mapping.
   *
   * @param flag the flag whose associated value is to be returned
   * @param backup the backup value to return if there is no mapping
   * @return the value to which the specified flag is mapped, or the backup value if there is no
   *     mapping
   */
  ...
  public String getString(String flag) {
    return this.map.getOrDefault(flag, null);
  }

  /**
   * Returns the value the specified flag is mapped as a {@link Path}, or the backup value if unable
   * to retrieve this mapping (including being unable to convert the value to a {@link Path} or if
   * no value exists).
   *
   ...
   * @return the value the specified flag is mapped as a {@link Path}, or the backup value if there
   *     is no valid mapping
  ...
  /**
   * Returns the value the specified flag is mapped as an int value, or the backup value if unable
   * to retrieve this mapping (including being unable to convert the value to an int or if no value
   * exists).
   *
   * @param flag the flag whose associated value will be returned
   * @param backup the backup value to return if there is no valid mapping
   * @return the value the specified flag is mapped as an int, or the backup value if there is no
   *     valid mapping
  ...
  /**
   * Returns the value the specified flag is mapped as an int value, or 0 if unable to retrieve this
   * mapping (including being unable to convert the value to an int or if no value exists).
   *
   * @param flag the flag whose associated value will be returned
   * @return the value the specified flag is mapped as an int, or 0 if there is no valid mapping
